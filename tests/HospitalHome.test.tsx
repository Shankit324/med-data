...
import Home from "@/app/hospital/[username]/page";
...
describe("Home Component", () => {
  it("renders Hospital Dashboard and sections correctly", async () => {
    ...
    expect(await screen.findByText("Hospital Dashboard")).toBeVisible();
    ...
    expect(anchorTag).toHaveAttribute("href", "/hospital/profile/testHospital");

    expect(await screen.findByText("View Report")).toBeVisible();
    anchorTag = links[1].closest("a");
    expect(anchorTag).toHaveAttribute("href", "/hospital/view_report/testHospital");

    expect(await screen.findByText("Add Report")).toBeVisible();
    anchorTag = links[2].closest("a");
    expect(anchorTag).toHaveAttribute("href", "/hospital/add_report/testHospital");
  });
