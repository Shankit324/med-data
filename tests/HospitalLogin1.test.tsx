...
import HospitalLogin from "@/app/hospital/login/page";
import React from "react";
import { describe, it, expect, vi } from "vitest";

// Mock `x_val` and `y_val` functions
vi.mock("@/app/hospital/login/submit", () => ({
  x_val: vi.fn((username) => Promise.resolve(username === "validUser")),
  y_val: vi.fn((username, password) => Promise.resolve(username === "validUser" && password === "correctPassword")),
}));
...
describe("HospitalLogin", () => {
  it("renders the login form", async () => {
    ...
    expect(await screen.findByText("Hospital Login")).toBeTruthy();
  ...
  it("shows error when username does not exist", async () => {
    ...
    await act(async () => {
      ...
      fireEvent.change(await screen.findByPlaceholderText("Password"), { target: { value: "anyPassword" } });
  ...
  it("shows error when password is incorrect", async () => {
    ...
    await act(async () => {
      ...
      fireEvent.change(await screen.findByPlaceholderText("Password"), { target: { value: "wrongPassword" } });
      fireEvent.click(await screen.findByText("Login"));
    });
  ...
  it("redirects on successful login", async () => {
    ...
    expect(window.location.href).toBe("/hospital/validUser");
  });
});
